...
#endif /* ?ZFREXPF */
int PVN_FABI(pvn_zdetf,PVN_ZDETF)(const float *const a, const float *const b, const float *const c, const float *const d, float *const x, int *const t, float *const h)
{
  PVN_ASSERT(a);
  PVN_ASSERT(b);
  PVN_ASSERT(c);
  ...
  PVN_ASSERT(h);
  register __m512
    fA = _mm512_loadu_ps(a),
    fB = _mm512_loadu_ps(b),
    fC = _mm512_loadu_ps(c),
    fD = _mm512_loadu_ps(d);
  ...
  register __m512 F = _mm512_set1_ps(-0.5f);
  W = _mm512_maskz_cvtepi32_ps(mC, J); /* W = T & 1 {mC} */
  register __m512 X = _mm512_set1_ps(0.5f);
  ...
  ZFREXPF(X, S, X, mD);
  T = _mm512_add_ps(T, S);
  _mm512_storeu_ps(x, X);
  J = _mm512_cvtps_epi32(T);
  _mm512_storeu_epi32(t, J);
  W = _mm512_scalef_ps(X, T);
  _mm512_storeu_ps(h, W);
...
#endif /* ?ZFREXP */
int PVN_FABI(pvn_zdet,PVN_ZDET)(const double *const a, const double *const b, const double *const c, const double *const d, double *const x, int *const t, double *const h)
{
  PVN_ASSERT(a);
  PVN_ASSERT(b);
  PVN_ASSERT(c);
  ...
  PVN_ASSERT(h);
  register __m512d
    fA = _mm512_loadu_pd(a),
    fB = _mm512_loadu_pd(b),
    fC = _mm512_loadu_pd(c),
    fD = _mm512_loadu_pd(d);
  ...
  register __m512d F = _mm512_set1_pd(-0.5);
  W = _mm512_maskz_cvtepi32_pd(mC, J); /* W = T & 1 {mC} */
  register __m512d X = _mm512_set1_pd(0.5);
  ...
  ZFREXP(X, S, X, mD);
  T = _mm512_add_pd(T, S);
  _mm512_storeu_pd(x, X);
  J = _mm512_cvtpd_epi32(T);
  _mm256_storeu_si256((__m256i*)t, J);
  W = _mm512_scalef_pd(X, T);
  _mm512_storeu_pd(h, W);
