...
/* an estimate of ||G||_F or ||off(G)||_F without hypot */
PVN_EXTERN_C void pvn_snrm2o_(const unsigned *const m, const unsigned *const n, const float *const g, const unsigned *const ldg, double *const ssq, int *const info);
PVN_EXTERN_C void pvn_dnrm2o_(const unsigned *const m, const unsigned *const n, const double *const g, const unsigned *const ldg, long double *const ssq, int *const info);
#ifdef PVN_QUADMATH
