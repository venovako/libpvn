...
- `nrm` uses the `mpfr_hypot`-based instead of the `mpfr_fma`-based norm computation;
...
So, `SAFE=ev2,jv2,sv2`, e.g., requests the `ev2`, `jv2`, and `sv2` safe implementations, but not the `nrm` nor the `ran` one.
